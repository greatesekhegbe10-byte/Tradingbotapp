...
import { Activity, Target, Crosshair, Map } from 'lucide-react';
      ...
      <div className="flex-1 w-full min-h-0 relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
            ...
            {/* SL/TP Reference Lines from AI Analysis */}
            {analysis && (
              <>
            ...
            {activeTrades.map((trade) => (
              <ReferenceLine 
                key={trade.id} 
                y={trade.price} 
                stroke={trade.type === 'CALL' || trade.type === 'BUY' ? '#10b981' : '#ef4444'} 
                strokeWidth={2}
                label={{ position: 'insideRight', value: 'ENTRY', fill: '#fff', fontSize: 8, fontWeight: 900, dy: -10 }} 
              />
            ))}
          </AreaChart>
        </ResponsiveContainer>
